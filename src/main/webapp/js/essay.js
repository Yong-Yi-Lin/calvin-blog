// 评论功能
    ...
    haveComment = function (commentView, arr) {
        ...
        arr.forEach(function (value) {
            htmlText +=
                            `<ul id="${value.id}" class="comment_ulist" style="width: 100%;">`+
                            `  <li class="comment_line_box" id="${value.id.substring(0, 6)}">` +
                            `    <div class="cmt_img">` +
                            `      <img alt="头像" class="avatar" src="images/login.png" width="30px" style="border-radius: 100%">` +
                            `    </div>` +
                            `    <div class="cmt_info">` +
                            `      <a class="commentator">${value.commentator}</a>` +
                            `      <span class="time" style="float:right">${value.time}</span>` +
                            `   </div>` +
                            `   <div class="cmt_reply" >` +
                            `       <span style="text-align: right;" id="${value.id.substring(0, 9)}">` +
                            `           <a class="cmt_reply_btn" style="margin-left: 70px;"id="${value.id.substring(0, 7)}">回复</a >` +
                            `       </span>`;
             
                        if(value.reply.length > 0) {
                            htmlText += `<a id="${value.id.substring(0, 8)}" class="cmt_reply_checker" style="float: right;">查看回复(${value.reply.length})</a>`
                        }
                        // 初始化当前角色是否给该条留言点赞了
                        const commentator = domById('role_info_name').innerText;
                        ...
                        htmlText += "" +
                            ...
                            `   </span>` +
                            `</div>`;
                        htmlText += ` <div style="display: block; margin-top: 8px; clear:both;" class="comment">${value.comment}</div>`;
                        htmlText += `</li></ul>`;
        });
    ...
    function thumbUp(value) {
        ...
        dom.onclick = function (ev) {
            ...
            if (isAdmired) {
                ...
                dom.style.color = 'rgba(255, 0, 0, 0.4)';
            } else {
                value.thumbUp += 1;
                dom.innerText = value.thumbUp.toString();
                dom.style.color = 'rgba(255, 0, 0, 0.9)';
            }
...
// 点赞功能
	...
	// ajax函数调回让点赞次数增加
// var diggnumElement = document.getElementById("diggnum");
// var count = 0;

// function makeRequest(url, callback, method, data) {
...
//       // 在成功的回调函数中更新计数和点赞数
//       count++;
//       diggnumElement.textContent = count;
//     }
//   }
//   xhr.send(data);
// }

// diggnumElement.addEventListener("click", function() {
//   count++;
//   diggnumElement.textContent = count;

//   // 调用 AJAX 请求的函数
//   makeRequest('/', 'EchoReturnedText', 'GET', '');
// });
